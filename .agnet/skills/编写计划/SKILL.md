...
编写全面的实现计划，假设工程师对代码库零上下文。记录他们需要知道的一切：每个任务要改哪些文件、代码、测试、相关文档、如何验证。把完整计划拆成小任务。DRY. YAGNI. TDD. 频繁提交。
...
> **给 Claude:** 必须使用 superpowers:executing-plans 逐任务实现此计划。
...
**2. 并行会话（单独）(Parallel Session)** - 新会话用 executing-plans，分批执行带检查点
