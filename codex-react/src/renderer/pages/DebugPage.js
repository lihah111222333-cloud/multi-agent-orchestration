...
function DebugPage() {
    ...
    const hasSentry = typeof window < "u" && !!window.electronBridge?.getSentryInitOptions;
    const canExportLogs = typeof window < "u" && !!window.electronBridge?.sendMessageFromView;
    const sessionId = typeof window < "u"
        ? window.electronBridge?.getSentryInitOptions?.()?.codexAppSessionId
        : undefined;

    const handleTriggerTestError = useCallback(async () => {
        const trigger = window.electronBridge?.triggerSentryTestError;
        if (trigger) {
            setIsTestingError(true);
            try { await trigger(); } finally { setIsTestingError(false); }
        }
    }, []);

    const handleExportLogs = useCallback(async (scope) => {
        const sendMessage = window.electronBridge?.sendMessageFromView;
        if (!sendMessage || isExporting) return;
        setIsExporting(true);
        try {
            await sendMessage({ type: "export-logs", scope });
        } catch (err) {
