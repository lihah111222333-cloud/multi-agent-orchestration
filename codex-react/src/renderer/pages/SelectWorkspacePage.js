 ...
 * 通过 MCP JSON-RPC config/write 写入后端配置
 *
 * @param {string[]} paths - 选中的 workspace 根目录路径
 * @returns {Promise<void>}
 */
function submitSelectedWorkspaces(paths) {
    return new Promise((resolve, reject) => {
        const id = `config/write:workspace-roots:${crypto.randomUUID()}`;
        const handler = (event) => {
        ...
        bridge.sendRequest({
            id,
            method: "config/write",
            params: { key: "workspace_roots", value: paths },
        });
    });
}
