            ...
            <Card.Content>
                ...
                <div className="flex flex-col gap-2">
                    {available.map((server) => (
                        <div key={server.id} className="flex items-center justify-between p-3 rounded-lg bg-token-bg-secondary">
                            <div>
    ...
    return (
        <Card>
            <Card.Content>
                ...
                <div className="flex flex-col gap-3">
                    <input
                        className="w-full bg-token-bg-secondary rounded-lg px-3 py-2 text-sm text-token-foreground border border-token-border outline-none"
                    ...
                    <div className="flex gap-2">
                        <button
                            className={`px-3 py-1 text-xs rounded-md transition-colors ${serverType === "url" ? "bg-token-primary text-white" : "bg-token-bg-secondary text-token-foreground"}`}
                            onClick={() => setServerType("url")}
                        >
                            URL (SSE)
                        </button>
                        <button
                            className={`px-3 py-1 text-xs rounded-md transition-colors ${serverType === "stdio" ? "bg-token-primary text-white" : "bg-token-bg-secondary text-token-foreground"}`}
...
function McpSettings() {
    ...
    const handleInstallRecommended = useCallback(async (recommended) => {
        // 通过 bridge.callAPI 添加推荐服务器
        try {
            await bridge.callAPI("config/value/write", {
                key: recommended.config.keyPath,
                value: recommended.config.value,
            });
        } catch (err) {
            console.error("[McpSettings] Install recommended failed:", err);
        }
    }, []);

    const handleAddServer = useCallback(async (serverConfig) => {
        try {
            await bridge.callAPI("config/value/write", {
            ...
            console.error("[McpSettings] Add server failed:", err);
        }
        setShowAddForm(false);
    }, []);

    const handleRemoveServer = useCallback(async (server) => {
        try {
            await bridge.callAPI("config/value/write", {
            ...
            console.error("[McpSettings] Remove server failed:", err);
        }
    }, []);
