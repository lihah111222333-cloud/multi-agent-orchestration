...
test('thread store sync is driven by bridge settle events', async () => {
  ...
  assert.equal(src.includes("eventType === 'thread/messages/page'"), true);
  assert.equal(src.includes("eventType === 'thread/compacted'"), true);
  assert.equal(src.includes("eventType === 'thread/tokenUsage/updated'"), true);
