...
func TestCalculateHydrationLoadLimit(t *testing.T) {
	tests := []struct {
		...
		{name: "exceeds_max", initialCount: 100, total: 99999, want: messagesconsumer.ThreadMessageHydrationMaxRecords},
		{name: "negative_initial", initialCount: -5, total: 10, want: 10},
		{name: "negative_initial_exceeds", initialCount: -5, total: 99999, want: messagesconsumer.ThreadMessageHydrationMaxRecords},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := messagesconsumer.CalculateHydrationLoadLimit(tt.initialCount, tt.total)
			if got != tt.want {
