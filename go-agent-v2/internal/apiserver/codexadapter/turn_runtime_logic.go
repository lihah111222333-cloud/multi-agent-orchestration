...
func ensureThreadReadyForTurnLogic(a *Adapter, ctx context.Context, threadID, cwd string) (*runner.AgentProcess, error) {
	...
	if proc, ok := ensureReadyRunningProcessLogic(a, ctx, manager, id, launchCwd); ok {
		return proc, nil
	}
	...
	resumeCandidates := collectResumeCandidatesLogic(a, ctx, id)
	...
	proc, err := ensureReadyLaunchProcessLogic(a, ctx, manager, id, launchCwd, startInstructions, dynamicTools)
	if err != nil {
		return nil, err
	}
	if len(resumeCandidates) == 0 {
		return ensureReadyNoResumeCandidatesLogic(a, id, proc), nil
	}

	resumed, lastResumeErr, fatalResumeErr := tryResumeHistoricalCandidatesLogic(a, ctx, manager, proc, id, launchCwd, resumeCandidates)
	...
	if lastResumeErr != nil {
		return ensureReadyResumeFallbackLogic(
			a,
			ctx,
			manager,
			id,
	...
	return ensureReadyNoHistoricalRolloutLogic(a, ctx, id, launchCwd, proc, len(resumeCandidates)), nil
}

func ensureReadyRunningProcessLogic(
	a *Adapter,
	ctx context.Context,
...
func ensureReadyLaunchProcessLogic(
	a *Adapter,
	ctx context.Context,
	manager *runner.AgentManager,
	agentID string,
...
func ensureReadyNoResumeCandidatesLogic(
	a *Adapter,
	agentID string,
...
func ensureReadyResumeFallbackLogic(
	a *Adapter,
	ctx context.Context,
	manager *runner.AgentManager,
	agentID string,
	...
	notifySessionLostLogic(a, agentID, lastResumeErr)
	registerBindingLogic(a, ctx, agentID, proc)
	return proc, nil
}

func ensureReadyNoHistoricalRolloutLogic(
	a *Adapter,
	ctx context.Context,
	agentID string,
	launchCwd string,
...
func registerBindingLogic(a *Adapter, ctx context.Context, agentID string, proc *runner.AgentProcess) {
...
func notifySessionLostLogic(a *Adapter, agentID string, lastErr error) {
	method, payload := BuildSessionLostNotification(agentID, lastErr)
	a.notify(method, payload)
}

func collectResumeCandidatesLogic(a *Adapter, ctx context.Context, agentID string) []string {
...
func tryResumeHistoricalCandidatesLogic(
	a *Adapter,
	ctx context.Context,
	manager *runner.AgentManager,
	proc *runner.AgentProcess,
	...
	for _, resumeThreadID := range resumeCandidates {
		...
		if err == nil {
			...
			registerBindingLogic(a, ctx, id, proc)
			return true, nil, nil
		}
		...
		if IsCodexProcessCrashError(err) {
			...
			notifySessionLostLogic(a, id, err)
...
func turnStartLogic(a *Adapter, ctx context.Context, req turnStartRequest) (turnStartEntryResult, error) {
	...
	prepared, err := prepareTurnStartSubmissionLogic(a, threadID, req.Input, selectedSkills, req.ManualSkillSelection)
	if err != nil {
		return turnStartEntryResult{}, err
	}
	...
	turnID, err := startTurnSubmissionAndTrackLogic(
		a,
		ctx,
		threadID,
		req.Cwd,
	...
	appendTurnStartUserTimelineLogic(a, ctx, prepared.TimelineAttachments, contracts.TurnAppendUserTimelineOptions{
		ThreadID:     threadID,
...
func turnSteerFromInputLogic(a *Adapter, req turnSteerRequest) (map[string]any, error) {
	...
	prepared, err := prepareTurnSteerSubmissionLogic(a, threadID, req.Input, selectedSkills, req.ManualSkillSelection)
	if err != nil {
		return nil, err
	}
...
func startTurnSubmissionAndTrackLogic(
	a *Adapter,
	ctx context.Context,
	threadID string,
	cwd string,
	...
	proc, err := ensureThreadReadyForTurnLogic(a, ctx, threadID, cwd)
	if err != nil {
		return "", err
	}
...
func resolveProcessLogic(a *Adapter, caller, threadID string) (*runner.AgentProcess, error) {
