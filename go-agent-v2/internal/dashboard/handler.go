...
func (s *Server) listInteractions(c *gin.Context) {
	if items, err := s.stores.Interaction.List(
		c.Request.Context(),
		c.Query("thread_id"),
		c.Query("keyword"),
		queryLimit(c, 100),
...
func (s *Server) createInteraction(c *gin.Context) {
	...
	if item, err := s.stores.Interaction.Create(c.Request.Context(), &store.Interaction{
...
func (s *Server) listTaskTraces(c *gin.Context) {
	if items, err := s.stores.TaskTrace.List(
		c.Request.Context(),
		c.Query("agent_id"),
		c.Query("keyword"),
		nil,
...
func (s *Server) listPromptTemplates(c *gin.Context) {
	if items, err := s.stores.PromptTemplate.List(
		c.Request.Context(),
		c.Query("agent_key"),
		c.Query("keyword"),
		queryLimit(c, 100),
...
func (s *Server) savePromptTemplate(c *gin.Context) {
	...
	if item, err := s.stores.PromptTemplate.Save(c.Request.Context(), &req); err != nil {
		serverError(c, err)
	} else {
		success(c, item)
	}
...
func (s *Server) listCommandCards(c *gin.Context) {
	if items, err := s.stores.CommandCard.List(c.Request.Context(), c.Query("keyword"), queryLimit(c, 100)); err != nil {
		serverError(c, err)
	} else {
		success(c, items)
	}
...
func (s *Server) saveCommandCard(c *gin.Context) {
	...
	if item, err := s.stores.CommandCard.Save(c.Request.Context(), &req); err != nil {
		serverError(c, err)
	} else {
		success(c, item)
	}
...
func (s *Server) listAuditLog(c *gin.Context) {
	if items, err := s.stores.AuditLog.List(
		c.Request.Context(),
		c.Query("event_type"),
		c.Query("action"),
		c.Query("actor"),
...
func (s *Server) listSystemLog(c *gin.Context) {
	if items, err := s.stores.SystemLog.ListV2(c.Request.Context(), store.ListParams{
		Level:     c.Query("level"),
...
func (s *Server) listAILog(c *gin.Context) {
	if items, err := s.stores.AILog.Query(c.Request.Context(), c.Query("category"), c.Query("keyword"), queryLimit(c, 100)); err != nil {
		serverError(c, err)
	} else {
		success(c, items)
	}
...
func (s *Server) listBusLog(c *gin.Context) {
	if items, err := s.stores.BusLog.List(
		c.Request.Context(),
		c.Query("category"),
		c.Query("severity"),
		c.Query("keyword"),
...
func (s *Server) listAgentStatus(c *gin.Context) {
	if items, err := s.stores.AgentStatus.List(c.Request.Context(), c.Query("status")); err != nil {
		serverError(c, err)
	} else {
		success(c, items)
	}
...
func (s *Server) listSharedFiles(c *gin.Context) {
	if items, err := s.stores.SharedFile.List(c.Request.Context(), c.Query("prefix"), queryLimit(c, 200)); err != nil {
		serverError(c, err)
	} else {
		success(c, items)
	}
...
func (s *Server) writeSharedFile(c *gin.Context) {
	...
	if item, err := s.stores.SharedFile.Write(c.Request.Context(), req.Path, req.Content, req.Actor); err != nil {
		serverError(c, err)
	} else {
		success(c, item)
	}
...
func (s *Server) deleteSharedFile(c *gin.Context) {
	if deleted, err := s.stores.SharedFile.Delete(c.Request.Context(), c.Param("path"), c.Query("actor")); err != nil {
		serverError(c, err)
	} else {
...
func (s *Server) listPendingApprovals(c *gin.Context) {
	if items, err := s.stores.TopologyApproval.GetPending(c.Request.Context()); err != nil {
		serverError(c, err)
	} else {
		success(c, items)
	}
...
func (s *Server) dbQuery(c *gin.Context) {
	...
	if rows, err := s.stores.DBQuery.Query(c.Request.Context(), req.SQL, req.Limit); err != nil {
...
func serverError(c *gin.Context, err error) {
	logger.FromContext(c.Request.Context()).Error("internal error", logger.FieldError, err)
