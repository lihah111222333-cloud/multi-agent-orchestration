...
func ArchiveThreadArtifacts(ctx context.Context, threadID string, resolveRolloutHistorySource func(context.Context, string) (string, string), bindRolloutPath func(context.Context, string, string, string)) (ThreadArchiveManifest, error) {
	...
	if resolveRolloutHistorySource != nil {
		...
		for _, candidate := range CollectThreadArtifactCandidates(manifest.CodexThreadID, rolloutPath) {
			...
			manifest.Files = append(manifest.Files, ThreadArchiveFile{Kind: candidate.Kind, SourcePath: srcPath, ArchivedPath: targetPath, SizeBytes: sourceState.SizeBytes, SHA256: checksum})
	...
	PruneArchivedCodexSourceFiles(id, manifest.Files, manifest.ArchiveDir, ResolveCodexRootDir, PathWithinRoot, FileSHA256, FileState, RemoveFile, RemoveEmptyCodexParentDirs)
	return manifest, nil
}
