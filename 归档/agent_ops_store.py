...
GO:migrate → go/internal/store/ops_store.go
GO:target  → Go sqlx + pgxpool
GO:notes:
  - 9 个 _row_to_* 函数 → Go struct tag `db:"col"` 自动映射 (消除 ~156 行)
  - 67x str(row.get()) → Go 类型系统编译时保证 (消除全部)
  - 17x SQL 列名重复 → Go 常量 + sqlx StructScan (消除 ~50 行)
  - 20x append_event 样板 → Go middleware/interceptor (消除 ~80 行)
  - SQL guard (_validate_read_only, _validate_execute, escape_like) → 等价移植 ~60 行 Go
  - RowMissingError → Go error sentinel: var ErrRowMissing = errors.New(...)
  - _normalize_key → Go func normalizeKey(name, val string) (string, error)
  - Connection pool (connect_cursor) → pgxpool.Pool 注入
"""
...
# GO:migrate-section → go/internal/store/sqlguard.go
# GO:notes: 以下常量和正则均需等价移植到 Go regexp.MustCompile (编译时)
...
# GO:migrate → var ErrRowMissing = errors.New("database returned no row")
...
def _is_db_execute_enabled() -> bool:
    raw = str(os.getenv("AGENT_DB_EXECUTE_ENABLED", "1")).strip().lower()
...
# GO:migrate → func normalizeKey(name, val string) (string, error)
...
# GO:migrate-eliminate → Go struct Interaction { ID int `db:"id" json:"id"` ... }
# GO:notes: 此函数及下方 8 个 _row_to_* 共 156 行，全部被 sqlx.StructScan 消除
...
# ─── Task ACK ───────────────────────────────────────────────────────
# GO:migrate → go/internal/store/task_ack.go
...
# ─── Task DAG ───────────────────────────────────────────────────────
# GO:migrate → go/internal/store/task_dag.go
