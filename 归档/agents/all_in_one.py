...
GO:migrate → go/cmd/mcp-server/main.go
GO:target  → mcp-go (github.com/mark3labs/mcp-go)
GO:notes:
  - 10 个工具函数 (iterm/shared_file/interaction/prompt_template/command_card/
    db/task/approval/lock/orchestration_tui/agent_watchdog) → Go mcp.Tool 注册
  - 103x json.dumps(ensure_ascii=False) → Go encoding/json 自动序列化
  - action if-elif 分发 (40 分支) → Go switch 或 dispatch map
  - _safe_json / _SafeEncoder → Go json.Marshal 原生处理 time.Time/Decimal
  - _locked_json_rw / flock → Go sync.Mutex 或 syscall.Flock
  - _singleton_lock → Go 文件锁 (syscall.Flock)
  - _parse_json → Go json.Unmarshal + error
  - iterm() → GO:keep-python (委托给 Python iterm_bridge 进程)
"""
...
def db(action: str = "query", sql: str = "", limit: int = 200) -> str:
    ...
    Args:
        action:
          ...
          - "schema"  返回所有表名、用途说明和行数估算（无需 sql 参数）
        sql: SQL 语句（schema 动作时不需要）
        limit: 查询结果限制（query 时）

    数据库表清单（请使用以下真实表名，勿猜测）:
        audit_events               — 审计事件日志（操作审计、变更记录）
        system_logs                — 系统运行日志（INFO/WARNING/ERROR）
        agent_interactions         — Agent 间消息/任务交互记录
        agent_status               — Agent 运行时状态快照（running/idle/stuck/error）
        shared_files               — Agent 间共享文件存储（KV 形式）
        prompts                    — Agent 提示词配置（agent_key + tool_name 绑定）
        prompt_templates           — 提示词模板库（可启用/禁用，带版本）
        prompt_versions            — 提示词模板版本归档（历史快照）
        command_cards              — 命令卡（可执行的运维/自动化脚本模板）
        command_card_runs          — 命令卡执行记录（含审批、输出、状态）
        command_card_versions      — 命令卡版本归档（历史快照）
        task_acks                  — 任务确认/进度跟踪（ACK 管理）
        task_dags                  — DAG 编排主表（任务有向无环图）
        task_dag_nodes             — DAG 节点（单个执行步骤，含依赖关系）
        task_traces                — 任务执行链路追踪（span 级别）
        topology_approvals         — 拓扑变更审批（架构变更提案）
        topology_approval_archives — 拓扑审批归档（已过期/已处理）
        bus_exception_logs         — ACP-BUS 异常日志（工具超时、崩溃等）
        schema_migrations          — 数据库迁移版本记录（勿操作）
    """
    action = action.strip().lower()

    if action == "schema":
        return _db_schema_info()
...
def _db_schema_info() -> str:
    """返回所有表名、用途说明和行数估算（db action=schema 的内部实现）。"""
    ...
    try:
        ...
        for r in rows:
            ...
            estimate = count_rows[0]["estimate"] if count_rows else -1
            tables.append({"table": name, "description": desc, "row_estimate": estimate})
        return json.dumps({"ok": True, "count": len(tables), "tables": tables}, ensure_ascii=False)
...
def main() -> None:
    ...
    # ── Codex 自动确认守护进程 ──
    _auto_confirm_proc = None
    if os.environ.get("CODEX_AUTO_CONFIRM", "1").strip().lower() not in ("0", "false", "no", "off"):
        _ac_script = Path(__file__).resolve().parent / "codex_auto_confirm.py"
        if _ac_script.exists():
            try:
                _ac_log = Path.home() / ".codex_auto_confirm.log"
                _auto_confirm_proc = subprocess.Popen(
                    [sys.executable, str(_ac_script), "run"],
                    stdout=open(str(_ac_log), "a"),
                ...
                _ac_pid_file.write_text(str(_auto_confirm_proc.pid))
                _logger.info("[acp-bus] codex_auto_confirm 已后台启动 (PID %d)", _auto_confirm_proc.pid)
            except Exception as _ac_err:
                _logger.warning("[acp-bus] codex_auto_confirm 启动失败: %s", _ac_err)
