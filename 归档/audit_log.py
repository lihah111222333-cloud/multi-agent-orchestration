...
GO:migrate → go/internal/store/audit_log.go
GO:target  → Go sqlx + slog 结构化日志
GO:notes:
  - _row_to_event → Go struct AuditEvent `db:"col"` (消除)
  - append_event → Go func AppendEvent(ctx, evt AuditEvent) error
  - query_events keyword LIKE → 等价移植 escape_like
  - list_filter_values → Go 等价 4 个 DISTINCT 查询
"""
