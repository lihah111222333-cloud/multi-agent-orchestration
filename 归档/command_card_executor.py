...
GO:migrate → go/internal/executor/command_card.go
GO:target  → Go os/exec + sqlx
GO:notes:
  - _row_to_run → Go struct CommandCardRun `db:"col"` (消除 ~20 行)
  - _validate_params → Go struct tag validation (消除 ~40 行)
  - subprocess.run → Go exec.CommandContext + context.WithTimeout
  - _detect_dangerous_pattern → Go regexp.MustCompile (编译时)
  - _recover_stale_runs → Go 等价 SQL
"""
