...
║  Go 对应: go-agent-v2/internal/dashboard/                    ║
╚══════════════════════════════════════════════════════════════╝
...
GO:migrate → go/internal/dashboard/
GO:target  → Go chi router + html/template
GO:notes:
  - render_html() 930 行内嵌 HTML → static/index.html 模板 + html/template
  - do_GET 639 行 if-elif → chi r.Get("/path", handler) 路由注册表
  - do_POST 881 行 if-elif → chi r.Post("/path", handler) 路由注册表
  - 每路由 ~15 行样板 (try/except/json.dumps/_respond) → Go 中间件统一处理
  - _respond() → Go render.JSON(w, r, data) 一行
  - _safe_content_length → Go http.MaxBytesReader 中间件
  - _build_agent_status_* → go/internal/monitor/
  - EventBus + SSE → go/internal/dashboard/sse.go
  - save_config / load_current_config → go/internal/config/
  - _serve_static → chi middleware.FileServer
  - main() → go/cmd/dashboard/main.go
"""
...
NAV_ITEMS = [
    ...
    ("autoconfirm", "自动确认", '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>'),
]
...
# GO:migrate → go/internal/dashboard/prompt_handler.go
...
<div class="shell">
    ...
    <main class="main">
        ...
        <!-- Auto-Confirm Page -->
        <div id="page-autoconfirm" class="page">
            <div class="card">
                <header class="card-header">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="18" height="18"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
                    <h2>Codex 自动确认</h2>
                    <span id="ac-status-badge" class="badge badge-gray" style="margin-left:auto;font-size:0.7rem">加载中</span>
                </header>
                <div class="card-body">
                    <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;flex-wrap:wrap">
                        <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                            <span style="font-size:0.85rem;color:var(--text-primary)">启用自动确认</span>
                            <input type="checkbox" id="ac-toggle" onchange="toggleAutoConfirm(this.checked)">
                        </label>
                        <span id="ac-pid" style="font-size:0.78rem;color:var(--text-muted);font-family:var(--font-mono)"></span>
                        <button type="button" class="btn btn-sm btn-secondary" onclick="loadAutoConfirmStatus()">刷新</button>
                    </div>
                    <h3 style="font-size:0.85rem;font-weight:600;margin-bottom:10px">运行日志 <span id="ac-log-lines" style="font-size:0.72rem;color:var(--text-muted);font-weight:400"></span></h3>
                    <div id="ac-log" style="max-height:500px;overflow-y:auto;border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;background:var(--bg-base);font-family:var(--font-mono);font-size:0.75rem;white-space:pre-wrap;color:var(--text-secondary);line-height:1.6">
                        加载中...
                    </div>
                </div>
            </div>
...
# GO:migrate → go/internal/dashboard/handler.go
# GO:notes: chi 路由注册表替代整个 if-elif 链
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    # GO:migrate → chi r.Get(...) 路由注册
    def do_GET(self) -> None:
        ...
        elif path == "/api/auto-confirm/status":
            _ac_pid_file = Path.home() / ".codex_auto_confirm.pid"
            _ac_log_file = Path.home() / ".codex_auto_confirm.log"
            running = False
            pid = None
            if _ac_pid_file.exists():
                try:
                    pid = int(_ac_pid_file.read_text().strip())
                    os.kill(pid, 0)
                    running = True
                except (ProcessLookupError, ValueError):
                    pid = None
            # read last N lines of log
            lines_count = _safe_int(params.get("lines", ["100"])[0], 100, 10, 500)
            log_lines = []
            if _ac_log_file.exists():
                try:
                    all_lines = _ac_log_file.read_text(encoding="utf-8", errors="replace").splitlines()
                    log_lines = all_lines[-lines_count:]
                except Exception:
                    pass
            payload = {
                "ok": True, "running": running, "pid": pid,
                "log": log_lines, "log_file": str(_ac_log_file),
            }
            self._respond(200, "application/json; charset=utf-8",
                          json.dumps(payload, ensure_ascii=False).encode("utf-8"),
                          headers={"Cache-Control": "no-store"})
    ...
    def do_POST(self) -> None:
        ...
        elif path == "/api/auto-confirm/toggle":
            try:
                data = json.loads(self.rfile.read(self._safe_content_length()).decode("utf-8"))
                enable = _safe_bool(data.get("enable", True), default=True)
                _ac_pid_file = Path.home() / ".codex_auto_confirm.pid"
                _ac_log_file = Path.home() / ".codex_auto_confirm.log"
                _ac_script = Path(__file__).resolve().parent / "agents" / "codex_auto_confirm.py"

                if enable:
                    # Check if already running
                    if _ac_pid_file.exists():
                        try:
                            old_pid = int(_ac_pid_file.read_text().strip())
                            os.kill(old_pid, 0)
                            self._respond(200, "application/json",
                                          json.dumps({"ok": True, "running": True, "pid": old_pid,
                                                      "message": "\u5df2\u5728\u8fd0\u884c\u4e2d"}, ensure_ascii=False).encode("utf-8"))
                            return
                        except (ProcessLookupError, ValueError):
                            _ac_pid_file.unlink(missing_ok=True)

                    import subprocess as _sp
                    proc = _sp.Popen(
                        [sys.executable, str(_ac_script), "run"],
                        stdout=open(str(_ac_log_file), "a"),
                        stderr=_sp.STDOUT,
                        stdin=_sp.DEVNULL,
                        start_new_session=True,
                    )
                    _ac_pid_file.write_text(str(proc.pid))
                    self._respond(200, "application/json",
                                  json.dumps({"ok": True, "running": True, "pid": proc.pid,
                                              "message": "\u5df2\u542f\u52a8"}, ensure_ascii=False).encode("utf-8"))
                else:
                    # Stop
                    if _ac_pid_file.exists():
                        try:
                        ...
                        _ac_pid_file.unlink(missing_ok=True)
                    self._respond(200, "application/json",
                                  json.dumps({"ok": True, "running": False, "pid": None,
                                              "message": "\u5df2\u505c\u6b62"}, ensure_ascii=False).encode("utf-8"))
            except Exception as e:
                self._respond(500, "application/json",
                              json.dumps({"ok": False, "error": str(e)}).encode("utf-8"))
