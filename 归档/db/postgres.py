...
GO:migrate → go/internal/db/pool.go
GO:target  → pgxpool.Pool (github.com/jackc/pgx/v5)
GO:notes:
  - psycopg3 pool → pgxpool (行业标杆, ~20 行初始化)
  - connect_cursor → pgxpool.Pool.Acquire/Begin
  - execute/fetch_all/fetch_one → sqlx.Select/Get/Exec
  - ensure_schema → golang-migrate 或自写 migrate_up
  - search_path 设置 → pgxpool AfterConnect callback
  - atexit close_pool → defer pool.Close()
"""
