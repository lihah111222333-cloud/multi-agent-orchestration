...
GO:migrate → go/internal/orchestrator/master.go
GO:target  → Go state machine + OpenAI API (替代 LangGraph)
GO:notes:
  - LangGraph StateGraph → Go struct + for-select 状态机 (~50 行替代)
  - _build_system_prompt → Go string template
  - run_orchestration_worker → Go goroutine
"""
