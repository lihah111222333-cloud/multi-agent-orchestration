...
/* ---- Auto-Confirm ---- */
async function loadAutoConfirmStatus() {
    try {
        const r = await fetch('/api/auto-confirm/status?lines=200');
        const j = await r.json();
        ...
        const pidEl = document.getElementById('ac-pid');
        const logEl = document.getElementById('ac-log');
        const linesEl = document.getElementById('ac-log-lines');

        if (badge) {
            badge.textContent = j.running ? '\u8fd0\u884c\u4e2d' : '\u5df2\u505c\u6b62';
            badge.className = 'badge ' + (j.running ? 'badge-green' : 'badge-red');
        }
        if (toggle) toggle.checked = j.running;
        if (pidEl) pidEl.textContent = j.pid ? `PID: ${j.pid}` : '';

        const lines = j.log || [];
        if (linesEl) linesEl.textContent = `(\u5171 ${lines.length} \u884c)`;
        if (logEl) {
            if (lines.length === 0) {
                logEl.textContent = j.running ? '\u65e5\u5fd7\u4e3a\u7a7a\uff0c\u7b49\u5f85\u4ea7\u751f...' : '\u65e5\u5fd7\u4e3a\u7a7a';
            } else {
                logEl.innerHTML = lines.map(line => {
                    const esc = escapeHtml(line);
                    if (line.includes('\u2705')) return `<span style="color:#22c55e">${esc}</span>`;
                    if (line.includes('\u274c') || line.includes('\u26a0\ufe0f')) return `<span style="color:#f59e0b">${esc}</span>`;
                    if (line.includes('\ud83d\ude80')) return `<span style="color:#3b82f6">${esc}</span>`;
                    if (line.includes('\ud83d\udfe5')) return `<span style="color:#8b5cf6">${esc}</span>`;
                    return esc;
                }).join('\n');
                logEl.scrollTop = logEl.scrollHeight;
            }
        }
    } catch (e) { toast('\u52a0\u8f7d\u81ea\u52a8\u786e\u8ba4\u72b6\u6001\u5931\u8d25: ' + e.message, false); }
}

async function toggleAutoConfirm(enable) {
    try {
        const r = await fetch('/api/auto-confirm/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const j = await r.json();
        if (j.ok) {
            toast(j.message || (enable ? '\u5df2\u542f\u52a8' : '\u5df2\u505c\u6b62'), true);
            setTimeout(loadAutoConfirmStatus, 500);
        } else {
            toast(j.error || '\u64cd\u4f5c\u5931\u8d25', false);
        }
    } catch (e) { toast('\u7f51\u7edc\u9519\u8bef: ' + e.message, false); }
